<ul class="todo-list mb-0 simple-bar">
    ...
    @endphp
        ...
        <li class="todo-list__items px-3 @if ($tasks->count() == '1')
            pt-5
        @endif">
            <div class="todo-list__items__wrapper d-xl-flex align-items-center justify-content-between">
                <div class="todo-list__items__wrapper__details d-flex align-items-center">
                    <div class="custom-control custom-checkbox mb-0">
                        <input value="{{ $task->id }}" type="checkbox" class="custom-control-input calendar-filter-checkbox taskCheckButton" id="{{ $task->id }}">
                    ...
                    <p
                        ...
                        data-clientId={{ $task->client_id }}
                        data-dueDate={{ $task->due_date }}
                        data-desc={{ $task->description }}
                        data-priority={{ $task->priority }} --}}
                        data-target="#rrrightAsideModal"
                        class="todo-title mb-0 ml-2 taskEditModal"
                        >
                        {{ $task->title }}
                    </p>
                    {{-- <p class="todo-title mb-0 ml-5">{{ $task->getClient->first_name }}</p> --}}
