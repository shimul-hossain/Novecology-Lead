<ul class="todo-list mb-0 simple-bar">
    @foreach ($important_tasks as $task)
    ...
    @endphp
        <li class="todo-list__items px-3 @if ($important_tasks->count() == '1')
            pt-5
        @endif">
            <div class="todo-list__items__wrapper d-xl-flex align-items-center justify-content-between">
                <div class="todo-list__items__wrapper__details d-flex align-items-center">
                    <div class="custom-control custom-checkbox mb-0">
                        ...
                        @endif
                        disabled
                        class="custom-control-input calendar-filter-checkbox  @if ($task->is_completed == 0)
                        taskCheckButton
                    @endif " id="{{ $task->id }}">
                        <label class="custom-control-label" for="{{ $task->id }}"></label>
                    </div>
                    <div class="custom-control custom-checkbox mb-0">
                        <span class="novecologie-icon-star taskImportantButton" style="color: #FF9F43" data-task-id="{{ $task->id }}"></span>
                    </div>
