...
@if ($filter_status->count() + $customFilterStatus->count() == 0)
    @forelse ($item_data as $lead) 
        <tr>
            <td>
                <div class="custom-control custom-checkbox"> 
                    <input type="checkbox"name="checkedLead[]" value="{{ $lead->id }}" class="custom-control-input table-select-checkbox lead_checkbox_item lead_checkbox_item__{{ $key }} progress_leads_checked" data-id="{{ $lead->id }}"  id="tableThreeRowSelectCheck-{{ $lead->id }}">
