...
@if ($project) 
    ...
    @php
        if($project->ProjectBareme->whereIn('id',  [7,29])->first()){
            $travauxs = $project->ProjectTravaux;
        }else{
            $travauxs = $project->ProjectBareme;
        } 
    @endphp
    ...
    @forelse ($travaux->travauxQuestion as $input) 
        @if ($input->type == 'textarea')
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    <textarea name="{{ $input->name }}" id="example_project{{ $input->id }}" class="form-control shadow-none question_disabled" @if ($input->required == 'yes') required @endif>{{ getCustomFieldData($input->name, $project->question_data) }}</textarea> 
                ...
                <div class="form-group">
                    ...
                    <select name="{{ $input->name }}" id="example_project{{ $input->id }}" class="custom-select shadow-none form-control question_disabled" @if ($input->required == 'yes') required @endif>
                        ...
                        @foreach (explode(',', $input->options) as $item)
                            <option @if ( trim(getCustomFieldData($input->name, $project->question_data)) == trim($item))
                                selected
                    ...
                    <div class="col-4">
                        <div class="form-group">
                            <div class="custom-control custom-checkbox">
                                <input type="radio" name="{{ $input->name }}" value="{{ $item }}" class="custom-control-input question_disabled" id="{{ $loop->iteration }}specify_heating{{ $input->id }}" @if ($input->required == 'yes') required @endif  @if (trim(getCustomFieldData($input->name, $project->question_data)) == trim($item))
                                checked
                            @endif>
                            <div class="invalid-feedback">{{ __('This field is necessary') }}</div>
                ...
                <div class="row">
                    ...
                    <div class="col-4">
                        <div class="form-group required-checkbox">
                            <div class="custom-control custom-checkbox">
                                <input @if (getFeature(getCustomFieldData($input->name, $project->question_data), $item))
                                checked
        ...
        @else
            <div class="col-md-6">
                <div class="form-group">
                    <label class="form-label" for="example_project{{ $input->id }}">{{ $input->title }} @if ($input->required == 'yes') <span class="text-danger">*</span> @endif </label>
                    <input type="{{ $input->type }}" name="{{ $input->name }}" id="example_project{{ $input->id }}" value="{{ getCustomFieldData($input->name, $project->question_data) }}" class="form-control shadow-none question_disabled" @if ($input->required == 'yes') required @endif>
                    <div class="invalid-feedback">{{ __('This field is necessary') }}</div>
                </div>
            </div>    
        @endif 
