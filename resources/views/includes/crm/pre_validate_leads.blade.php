@if ($filter_status->count() + $customFilterStatus->count() == 0)
    @foreach ($pre_validate_leads as $lead)
    @if (role() != 's_admin')
        @if (!getLeadAccess(Auth::id(), $lead->id))
            @continue
        @endif
    @endif
        <tr>
            <td>
                <div class="custom-control custom-checkbox">
                    <input type="checkbox" class="custom-control-input table-select-checkbox pre_validate_lead_checked" data-id="{{ $lead->id }}"  id="tableThreeRowSelectCheck-{{ $lead->id }}">
            ...
            <td>
                <div class="avatar-group d-flex">
                    ...
                    <div class="avatar-group__image-wrapper position-relative flex-shrink-0 rounded-circle overflow-hidden" data-toggle="tooltip" data-placement="top" title="{{ getAssigneePhoto($item->user_id)->name }}"> 
...
@else
    ...
    <tr>
        <td>
            <div class="custom-control custom-checkbox">
                <input type="checkbox" name="checkedLead[]" value="{{ $lead->id }}" class="custom-control-input table-select-checkbox pre_validate_lead_checked" data-id="{{ $lead->id }}" id="tableThreeRowSelectCheck-{{ $lead->id }}">
                <label class="custom-control-label" for="tableThreeRowSelectCheck-{{ $lead->id }}"></label>
            </div>
        </td> 
        @foreach ($filter_status as $item)
            ...
            @endphp
            @if($header == 'ID')
            <td> <a href="{{ route('leads.index',[$lead->company_id ,$lead->id]) }}">{{ $lead->id??__('Not Provided') }}</a></td>
            @endif
