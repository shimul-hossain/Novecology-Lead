...
{{-- Title --}}
@section('title')
{{ config('app.name') }} | {{ __('Banner Create') }}
@endsection
    ...
    <div class="row breadcrumbs-top">
        <div class="col-12">
            ...
            <div class="breadcrumb-wrapper">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item">{{ __('Banner Create') }}</li>
                </ol>
            </div>
        </div>
    </div>
                ...
                <div class="card-body">
                    <form action="{{ route('banner.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            <label for="first_line">{{ __('Title') }}</label>
                            <textarea name="first_line" class="form-control" id="first_line"></textarea>
                            @error('text')
                        ...
                        <div class="form-group">
                            <label for="second_line">{{ __('Sub Title') }}</label>
                            <textarea name="second_line" class="form-control" id="second_line" ></textarea>
                            @error('text')
                            <span class="text-danger">{{$message}}</span>
                            @enderror
                        </div>
