            ...
            <div class="row justify-content-center">
                    ...
                    <div class="user-card mb-5">
                        <div class="user-card__head position-relative">
                            @if (checkAction(Auth::id(), 'project', 'pixel-create') || $role == 's_admin')
                                <button type="button" class="btn shadow-none user-card__head__wrapper_pixel_btn" data-toggle="modal" data-target="#pixelModal">
                                    <img width="45" height="40" src="{{ asset('crm_assets/assets/images/pixel-updated.png') }}" alt="">
                                </button> 
                            @endif
                            <div class="user-card__head__wrapper position-relative">
                                ...
                                @if (checkProjectDuplicateEntry($project))
                                    <div class="alert bg-danger text-white">
                                        <div class="alert-body text-center"><a target="_blank" href="{{ route('project.similar.file', $project->id) }}">
                                            Attention, dossier(s) similaire(s) déja saisi(s).
                                        </a></div>
                                    </div>
                                @endif 
                                ...
                                <div class="text-center d-flex flex-column px-md-5">
                                    @if($project->client_id)
                                        <button type="button" class="primary-btn primary-btn--white primary-btn--lg rounded-pill d-inline-flex align-items-center justify-content-center border-0 mt-2">
                                            <a href="{{ route('client.lead.update', $project->client_id) }}"><span class="novecologie-icon-eye px-2"> </span>  {{ __('Consulter la fiche client') }}</a>
                                        </button>
                                    @endif
                                    ...
                                    <div class="arrow-btn-group d-none" id="left_side_button">
                                        @if ($project->Type_de_contrat && $project->Faisabilité_du_projet && $project->Statut_Projet || ($project->Type_de_contrat == 'BAR TH 173' && $project->project_label != 5 && $project->project_label != 6))
                                            @foreach ($project_statuses as $p_status)
                                                @if ($p_status->id == $project->project_label)
                                                    @php
                                                        $convert_status = true;
                                                        $convert_status2 = true;
                                                        if((!$bareme_status && (!$project->primaryTax || $project->primaryTax->tax_number == '0000000000')) && ($p_status->id == '3' || $p_status->id == '4' || $p_status->id == '5' || $p_status->id == '6' || $p_status->id == '8')){
                                                            $convert_status = false;
                                                        }
                                                        if($project->getSubventions->first() && (!$project->getSubventions->first()->subvention_status || $project->getSubventions->first()->subvention_status == 'Demande de subvention déposé' || $project->getSubventions->first()->subvention_status == 'En cours d’instruction' || $project->getSubventions->first()->subvention_status == 'Dépôt de subvention en attente de complément') && ($p_status->id == '4' || $p_status->id == '5' || $p_status->id == '6' || $p_status->id == '8')){
                                                            $convert_status2 = false;
                                                        }
                                                    @endphp
                                                    <button @if (!$convert_status)
                                                                data-toggle="tooltip" data-placement="top" title="Avis d’Impot pas réel"
                                                            @elseif (!$convert_status2)
                                                                data-toggle="tooltip" data-placement="top" title="Statut MPR - La subvention est invalide"
                                                            @else
                                                                @if (checkAction(Auth::id(), 'project', 'statut_edit') || role() == 's_admin') 
                                                                    data-toggle="modal" data-target="#project_status__change" 
                                                                @endif
                                                            @endif type="button" data-label-text="{{ $p_status->status }}" data-label-id="{{ $p_status->id }}" class="arrow-btn m-auto 
                                        ...
                                        @else
                                            @foreach ($project_statuses as $p_status)
                                                @if ($p_status->id == $project->project_label)
                                                    <button @if ($p_status->id != 1)
                                                                data-toggle="tooltip" data-placement="top" title="Type de contrat, Faisabilité du projet, statut projet certains d'entre eux manquent"
                                                            @else
                                                                @if (checkAction(Auth::id(), 'project', 'statut_edit') || role() == 's_admin') 
                                                                    data-toggle="modal" data-target="#project_status__change" 
                                                                @endif
                ...
                <div class="lead__column position-relative col-xl mt-xl-0">
                    <div class="arrow-btn-group">
                        @if ($project->Type_de_contrat && $project->Faisabilité_du_projet && $project->Statut_Projet || ($project->Type_de_contrat == 'BAR TH 173' && $project->project_label != 5 && $project->project_label != 6))
                            @foreach ($project_statuses as $p_status)
                                ...
                                <button @if (!$convert_status)
                                        ...
                                        @elseif (!$convert_status2)
                                        data-toggle="tooltip" data-placement="top" title="Statut MPR - La subvention est invalide"
                                    @endif type="button" data-label-text="{{ $p_status->status }}" data-label-id="{{ $p_status->id }}" class="arrow-btn top_section_button 
                                    @if (checkAction(Auth::id(), 'project', 'statut_edit') || role() == 's_admin')
                                        @if ($convert_status && $convert_status2)
                                            projectLabelChangeBtn 
                                        @endif
                                    @endif
                                    " style="color:{{ $project->project_label == $p_status->id ? $p_status->background_color : '#ffffff' }};">
                                    <span class="arrow-btn__text" style="color: {{ $project->project_label == $p_status->id ? '':'#1a1a1a' }}">{{ $p_status->status }}</span>
                                </button>
                            @endforeach
                        @else
                            @foreach ($project_statuses as $p_status)
                        ...
                        @endif
                        @if (role() == 's_admin')
                            <button type="button" data-toggle="modal" data-target="#projectSingleDeleteModal"  class="btn btn-icon shadow-none ml-auto mt-auto text-white">
                                <i class="bi bi-trash3"></i>
                            </button>  
                            <div class="modal modal--aside fade" id="projectSingleDeleteModal" tabindex="-1" aria-labelledby="TicketMessageStoreModal" aria-hidden="true">
                    ...
                    <div class="row">
                        {{-- <div class="col-md-6">
                            <p class="accessablity-text text-shadow mb-md-0">{{ __('Subscription date') }}: <span class="subscription-date">{{ \Carbon\Carbon::parse($project->created_at)->format('d/m/Y') }}</span></p>
                        </div> --}}
                        <div class="col-12 text-md-right">
                            <button data-toggle="{{ $project->phone ? 'modal':'' }}"  data-target="#callBackModal" class="primary-btn primary-btn--pink primary-btn--sm rounded-pill d-inline-flex align-items-center justify-content-center border-0" type="button">@if ($project->callback_time && \Carbon\Carbon::parse($project->callback_time) > \Carbon\Carbon::now())
                                Rappel en cours
                                @else
                                Rappeler
                                @endif</button>
...
<script>
	...
	$(document).ready(function(){ 
        ...
        $('body').on('click','#travauxValidate',function(){
            ...
            //     $('#errorMessage').html("Le champ produit est requis");
			// 	$('.toast.toast--error').toast('show');
			// 	product_error.focus();
			...
			// }else if($('#Statut_Projet').val() ==' '){
            //     $('#errorMessage').html("Le champ statut projet est requis");
			// 	$('.toast.toast--error').toast('show');
			// 	$('#Statut_Projet').focus();
            }else{
                $.ajaxSetup({
        ...
        $('body').on('click','#compteValidate',function(){
            ...
            if(compte_email_status == 'Oui' && email_email == ''){
                $('#errorMessage').html("Le champ email est requis");
				$('.toast.toast--error').toast('show');
                $('#email_email').focus();
            }
			else if(compte_email_status == 'Oui' && !regex.test(email_email)){
				$('#errorMessage').html("Veuillez entrer un courriel valide");
				$('.toast.toast--error').toast('show');
				$('#email_email').focus();
			}
            else if(compte_email_status == 'Oui' && password_email == ''){
                $('#errorMessage').html("Le champ mot de passe est requis");
				$('.toast.toast--error').toast('show');
                $('#password_email').focus();
            }
            else if(compte_email_status == 'Oui' && $('#account_created_on').val() == ''){
                $('#errorMessage').html("Le champ compte le crée est requis");
				$('.toast.toast--error').toast('show');
                $('#account_created_on').focus();
            }
            else if(compte_email_status == 'Oui' && $('#account_created_by').val() == ''){
                $('#errorMessage').html("Le champ compte crée par est requis");
				$('.toast.toast--error').toast('show');
                $('#account_created_by').focus();
            }
            else if(compte_email_recovery_status == 'Oui' && Compte_Email_de_récupération_email == ''){
                $('#errorMessage').html("Le champ email est requis");
				$('.toast.toast--error').toast('show');
                $('#Compte_Email_de_récupération_email').focus();
            }
			else if(compte_email_recovery_status == 'Oui' && !regex.test(Compte_Email_de_récupération_email)){
				$('#errorMessage').html("Veuillez entrer un courriel valide");
				$('.toast.toast--error').toast('show');
				$('#Compte_Email_de_récupération_email').focus();
			}
            else if(compte_email_recovery_status == 'Oui' && Compte_Email_de_récupération_Mots_de_passe == ''){
                $('#errorMessage').html("Le champ mot de passe est requis");
				$('.toast.toast--error').toast('show');
                $('#Compte_Email_de_récupération_Mots_de_passe').focus();
            }
            else if(compte_email_recovery_status == 'Oui' && $('#Compte_Email_de_récupération_crée_le').val() == ''){
                $('#errorMessage').html("Le champ compte le crée est requis");
				$('.toast.toast--error').toast('show');
                $('#Compte_Email_de_récupération_crée_le').focus();
            }
            else if(compte_email_recovery_status == 'Oui' && $('#Compte_Email_de_récupération_crée_par').val() == ''){
                $('#errorMessage').html("Le champ compte crée par est requis");
				$('.toast.toast--error').toast('show');
                $('#Compte_Email_de_récupération_crée_par').focus();
            }
            else if($('#Téléphone_de_récupération').val() == ''){
                $('#errorMessage').html("Le champs téléphone de récupération est requis");
				$('.toast.toast--error').toast('show');
                $('#Téléphone_de_récupération').focus();
            }
            else if($('#Email_de_transfert').val() == ''){
                $('#errorMessage').html("Le champ email de transfert est requis");
				$('.toast.toast--error').toast('show');
                $('#Email_de_transfert').focus();
            }
            else if(compte_MaPrimeRénov_status == 'Oui' && email_mpr == ''){
                $('#errorMessage').html("Le champ email est requis");
				$('.toast.toast--error').toast('show');
                $('#email_mpr').focus();
            }
			else if(compte_MaPrimeRénov_status == 'Oui' && !regex.test(email_mpr)){
				$('#errorMessage').html("Veuillez entrer un courriel valide");
				$('.toast.toast--error').toast('show');
				$('#email_mpr').focus();
			}
            else if(compte_MaPrimeRénov_status == 'Oui' && password_mpr == ''){
                $('#errorMessage').html("Le champ mot de passe est requis");
				$('.toast.toast--error').toast('show');
                $('#password_mpr').focus();
            }
            else if(compte_MaPrimeRénov_status == 'Oui' && $('#account_email_created_on').val() == ''){
                $('#errorMessage').html("Le champ compte le crée est requis");
				$('.toast.toast--error').toast('show');
                $('#account_email_created_on').focus();
            }
            else if(compte_MaPrimeRénov_status == 'Oui' && $('#account_email_created_by').val() == ''){
                $('#errorMessage').html("Le champ compte crée par est requis");
				$('.toast.toast--error').toast('show');
                $('#account_email_created_by').focus();
            }
            else{
        ...
        $('body').on('click','#information2Validate',function(){
            ...
            if(!$('#Number_Dossier_Action_Logement').val()){
                $('#errorMessage').html("Le champ N° Dossier Action Logement est obligatoire");
				$('.toast.toast--error').toast('show');
				$('#Number_Dossier_Action_Logement').focus();
            }else if(!$('#Montant_subvention_prévisionnelle').val()){
                $('#errorMessage').html("Montant subvention prévisionnelle champ est requis");
				$('.toast.toast--error').toast('show');
				$('#Montant_subvention_prévisionnelleInput').focus();
            }else if(!$('#Travaux_déposés').val()){
                $('#errorMessage').html("Le champ travaux déposés est requis");
				$('.toast.toast--error').toast('show');
				$('#Travaux_déposés').focus();
            }else if(!$('#Statut_Action_logement').val()){
                $('#errorMessage').html("Le champ statut action logement est requis");
				$('.toast.toast--error').toast('show');
				$('#Statut_Action_logement').focus();
            }else if(!$('#amo').val()){
                $('#errorMessage').html("Le champ AMO est requis");
				$('.toast.toast--error').toast('show');
				$('#amo').focus();
            }else{
                $.ajaxSetup({
