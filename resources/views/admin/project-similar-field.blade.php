...
@forelse ($similar_projects as $project)
    ...
    @elseif (strtolower($project->$field) != strtolower($field_data))
        </tbody>
    ...
    <tr> 
        <td>
            <div class="custom-control custom-checkbox">
                <input type="checkbox" data-project-id="{{ $project->id }}" class="custom-control-input table-select-checkbox similarProjectCheckboxBtn" id="similar_projects-{{ $project->id }}">
                <label class="custom-control-label" for="similar_projects-{{ $project->id }}"></label>
            </div>
        </td>
        <td>{{ "BH".sprintf('%08d', $project->id) }}</td>
        ...
        <td>{{ $project->phone }}</td>
        <td>{{ $project->projectStatus->status ?? '' }}</td>
        ...
        @if (checkAction(Auth::id(), 'project', 'similar-chantier-link') || role() == 's_admin')
            <td>
                <div class="d-flex">
                    ...
                    @if (role() == 's_admin')
                        <button type="button" data-toggle="modal" data-target="#projectSingleDeleteModal{{ $project->id }}"  class="btn btn-icon shadow-none ml-auto mt-auto">
