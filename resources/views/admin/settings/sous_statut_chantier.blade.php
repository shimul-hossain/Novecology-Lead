...
@section('SousStatutChantierTabActive', 'active')

@section('tab-content')
    <div class="tab-pane fade show active" id="v-pills-project_sub_status" role="tabpanel" aria-labelledby="v-pills-project_sub_status-tab">
                            ...
                            <tbody class="database-table__body">
                                @foreach ($project_sub_statuses as $project_sub_status)
                                    <tr>
                                        ...
                                        <td>
                                            @foreach ($project_sub_status->getStatus as $status)
                ...
                <div class="modal-body">
                    ...
                    <form action="{{ route('project.sub-status.update') }}" class="form mx-auto needs-validation"  novalidate method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <select class="select2_select_option form-control" name="status_id[]" multiple>
                                @foreach ($project_statuses as $status)
                                    <option {{ $project_sub_status->getStatus->where('id', $status->id)->first() ? 'selected':'' }} value="{{ $status->id }}">{{ $status->status }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group mt-4 d-flex justify-content-between">
                            ...
                            @if (checkAction(Auth::id(), 'general__setting-project_sub_status', 'delete') && role() == 's_admin')
                                <button type="button" class="btn btn-danger rounded border-0 mb-3 shadow-none" data-dismiss="modal" aria-label="Close" data-toggle="modal" data-target="#project_sub_status_delete{{ $project_sub_status->id }}">
