{{-- Main Layouts --}}
@extends('layouts.optimized-master')

{{-- Title part  --}}
@section('title', 'Magic planning')

@section('bodyBg', 'secondary-bg')

{{-- active menu  --}}
@section('magicPlanning')
active
@endsection

@include('includes.plugins.select2.index')

{{-- @push('plugins-script')
<script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyDgjBuD_z2DYaMscUhT16yzCve_7n1CQ_E&callback=initMap" defer async></script>
@endpush --}}
...
@section('content')
    <section class="pt-2">
        <div class="container">
            <form action="{{ route('magic.planning') }}" class="row mt-3" method="get">
                <div class="col-12">
                    <div class="row"> 
                        <div class="col-lg-10">
                            <div class="row">
                                <div class="col-md-6 col-xl-3">
                                    <div class="form-group">
                                        <label for="project" class="form-label mr-2 mb-0">Client*</label>
                                        <select id="project" name="project" class=" custom-select shadow-none form-control select2_select_option" required>
                                            <option value="" selected>{{ __("Select") }}</option>
                                            @foreach ($all_projects as $project)
                                                <option {{ request()->project == $project->id ? 'selected':''  }} value="{{ $project->id }}">{{ $project->Prenom .' '. $project->Nom }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                <div class="col-md-6 col-xl-3">
                                    <div class="form-group">
                                        <label for="sub_status" class="form-label mr-2 mb-0">Statut </label>
                                        <select id="sub_status" data-placeholder="{{ __('Select') }}" name="sub_status[]" class=" custom-select shadow-none form-control select2_select_option" multiple>
                                            @foreach ($project_sub_status as $sub_status)
                                                <option @if (isset(request()->sub_status) && in_array($sub_status->id, request()->sub_status))
                                                    selected
                                ...
                                <div class="col-md-6 col-xl-3">
                                    <div class="form-group">
                                        <label for="status" class="form-label mr-2 mb-0">Etiquette </label>
                                        <select id="status" name="status" class=" custom-select shadow-none form-control select2_select_option">
                                            <option value="" selected>{{ __("Select") }}</option>
                                            @foreach ($statuses as $status)
                                                <option {{ request()->status == $status->id ? 'selected':'' }} value="{{ $status->id }}">{{ $status->status }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                <div class="col-md-6 col-xl-3">
                                    <div class="form-group">
                                        <label for="travaux" class="form-label mr-2 mb-0">Travaux </label>
                                        <select id="travaux" data-placeholder="{{ __('Select') }}" name="travaux[]" class=" custom-select shadow-none form-control select2_select_option" multiple> 
                                            @foreach ($travauxs as $travaux)
                                                <option @if (isset(request()->travaux) && in_array($travaux->id, request()->travaux))
                                                    selected
                                                @endif value="{{ $travaux->id }}">{{ $travaux->travaux }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="col-md-6 col-xl-3">
                                    <div class="form-group ">
                                        <label for="type_de_contrat" class="form-label mr-2 mb-0">Type de contrat </label>
                                        <select id="type_de_contrat" name="type_de_contrat" class="custom-select shadow-none form-control select2_select_option">
                                            <option value="" selected>{{ __('Select') }}</option>
                                            <option {{ request()->type_de_contrat == 'Credit' ? 'selected':'' }} value="Credit">Credit</option>
                                            <option {{ request()->type_de_contrat == 'MaPrimeRenov' ? 'selected':'' }} value="MaPrimeRenov">MaPrimeRenov</option>
                                            <option {{ request()->type_de_contrat == 'BAR TH 164 – 2022' ? 'selected':'' }} value="BAR TH 164 – 2022">BAR TH 164 – 2022</option>
                                            <option {{ request()->type_de_contrat == 'BAR TH 164 – 2023' ? 'selected':'' }} value="BAR TH 164 – 2023">BAR TH 164 – 2023</option>
                                            <option {{ request()->type_de_contrat == 'BAR TH 164 – 2023 (après 01/07/2023)' ? 'selected':'' }} value="BAR TH 164 – 2023 (après 01/07/2023)">BAR TH 164 – 2023 (après 01/07/2023)</option>
                                            <option {{ request()->type_de_contrat == 'BAR TH 145' ? 'selected':'' }} value="BAR TH 145">BAR TH 145</option>
                                            <option {{ request()->type_de_contrat == 'BAR TH 173' ? 'selected':'' }} value="BAR TH 173">BAR TH 173</option>
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="col-md-6 col-xl-3">
                                    <div class="form-group ">
                                        <label for="bareme" class="form-label mr-2 mb-0">Barème</label>
                                        <select id="bareme" data-placeholder="{{ __('Select') }}"  name="bareme[]" class="custom-select shadow-none form-control select2_select_option" multiple> 
                                            @foreach ($travauxs as $bareme)
                                                <option @if (isset(request()->bareme) && in_array($bareme->id, request()->bareme))
                                                    selected
                                                @endif value="{{ $bareme->id }}">{{ $bareme->bareme }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
...
<script>
    $(document).ready(function () {
        ...
        window.initMap = function() {
            ...
            @if (isset($project_info['latitude']))
                allLocations.push({
