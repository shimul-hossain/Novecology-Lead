...
@endforeach  --}}
{{-- @php
    dd($tag_product_nombre);
@endphp --}}
@foreach ($barame_travaux_tags as $tag) 
    @if ($tagList && in_array($tag->id, $tagList) && $tag->id != 7 && $tag->id != 29) 
        ...
        @if ($tag->tag == 'CAG' || $tag->tag == 'POELE' || $tag->tag == 'PAC R/R' || $tag->tag == 'PAC R/O' || $tag->tag == 'CESI' || $tag->tag == 'BTD' || $tag->tag == 'SSC')
            <div class="form-group">
                ...
                <select name="marque[{{ $tag->id }}]" id="marque{{ $tag->id }}" class="select2_select_option shadow-none form-control travaux_disabled prjectMarquelist" data-tag-id="{{ $tag->id }}">
                    <option value="">{{ __("Select") }}</option>
                    @foreach ($marques as $marque_item)
                        @if(in_array($marque_item->id, $tag->getProducts->pluck('marque_id')->toArray()))
                            <option  
                                @if ($marque && isset($marque[$tag->id]) && $marque[$tag->id] == $marque_item->id)
                                    selected
        ...
        @if ($tag->tag == 'PAC R/R') 
            <div class="form-group">
                ...
                <input type="number" name="Nombre_de_split[{{ $tag->id }}]" @if ($Nombre_de_split && isset($Nombre_de_split[$tag->id]))
        ...
        @if ($tag->rank == '1')
            <div class="form-group">
                ...
                <select id="product{{ $tag->id }}" name="tag_product[{{ $tag->id }}][]" data-tag-id="{{ $tag->id }}" class="select2_select_option custom-select shadow-none form-control travaux_disabled  project-tag-product--change"  data-tag-id="{{ $tag->id }}" multiple>  
        ...
        @if ($tag->tag == 'ITI_101' || $tag->tag == 'ITI_102' || $tag->tag == 'ITE_102' || $tag->tag == 'ITI_103'|| $tag->tag == 'Crépis' || $tag->tag == 'ITE hors zone') 
            @if ($tag->tag == 'ITI_101')
                <div class="form-group">
                    ...
                    <select name="Type_de_comble[{{ $tag->id }}]" id="Type_de_comble{{ $tag->id }}" class="select2_select_option custom-select shadow-none form-control travaux_disabled">
            ...
            <div class="form-group">
                ...
                <input type="hidden" name="surface[{{ $tag->id }}]" @if ($surface && isset($surface[$tag->id]))
