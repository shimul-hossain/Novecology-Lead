...
@section('content')
			...
			<div class="container">
				...
				<div class="row justify-content-center">
					...
					<div class="col-lg-auto d-flex flex-wrap align-items-center justify-content-center justify-content-lg-right">
							...
							<div class="dropdown d-none" id="allActionButton">
								...
								<div class="dropdown-menu">
									@if ($filter_telecommercial_status)
										<button type="button" class="dropdown-item border-0" data-toggle="modal" data-target="#projectGestaionnaire">
											Attribuer gestionnaire
										</button>
										<form action="{{ route('project.gestionnaire.unassign') }}" method="POST">
											@csrf
											<input type="hidden" name="checkedProject" class="bulk_selected_project">
											<button type="submit" class="dropdown-item border-0">
												Désattribuer gestionnaire
											</button>
										</form>
									@endif

									<button type="button" class="dropdown-item border-0" data-toggle="modal" data-target="#projectTelecommercial">
										Attribuer un télécommercial
									</button>
									<form action="{{ route('project.telecommercial.unassign') }}" method="POST">
										@csrf
										<input type="hidden" name="checkedProject" class="bulk_selected_project">
										<button type="submit" class="dropdown-item border-0">
									...
									@if ($role == 's_admin')
										<button type="button" class="dropdown-item border-0" data-toggle="modal" data-target="#projectBulkDeleteModal">
											Supprimer chantier
										</button>
									@endif
									{{-- @if (checkAction(Auth::id(), 'project', 'magic-planning') || $role == 's_admin')
										<button type="button" class="dropdown-item border-0" id="singleProjectLocation">Magic planning</button>
									@endif --}}
								</div>
							</div>
						@endif
						@if ($role == 's_admin') 
							<a href="{{ route('project-import.index') }}" class="primary-btn primary-btn--white primary-btn--md rounded d-inline-flex align-items-center justify-content-center border-0 ml-2">Imported chantier 
							</a>
						@endif
						...
						@endif
						@if ($user_action_import || $role == 's_admin')
						...
						@if ($user_action_add_filter || $role == 's_admin')
							<button data-toggle="modal" data-target="#rightAsideModal" type="button" class="secondary-btn border-0 rounded shadow-none mx-2">+ {{ __('Add filter') }}</button>
						@endif
						@if ($user_action_filter_blue_button || $role == 's_admin')
							<button id="hugeFilterModalBtn" type="button" class="btn border-0 rounded" style="background-color:#42a5f6">
							...
							@foreach ($project_status as $key => $item_data)
								<li class="nav-item" role="presentation" data-tab-id="{{ $key }}">
									...
									@else
										...
										@endif
									@endif
									  class="nav-link border-0 {{ $status ? ($item_data->id == $status ? 'active':''): ($item_data->id == '1'? 'active':'') }}"  href="{{ route('project.index', $item_data->id) }}"> {{ $item_data->status }} @if ($item_data->id == $status)<span class="border ml-2 p-1 rounded-pill">{{ $projects->total() }}</span>@endif</a>
								</li>
							@endforeach
							<li class="ml-auto">
								@if (checkAction(Auth::id(), 'project', 'similar-chantier') || $role == 's_admin')
									<button type="button" class="primary-btn primary-btn--white primary-btn--sm rounded d-inline-flex align-items-center justify-content-center border-0" data-toggle="modal" data-target="#similarProjectModal">
										Analyse doublons
									</button>
								@endif
													...
													@forelse ($projects as $project)
														...
														<tr>
															...
															@foreach ($default_filters as $item)
																...
																@endphp
																@if ($header == 'Nom' && $project->pinComment)
																	<td id="{{ $header }}{{ $project->id }}">
																		<span data-toggle="tooltip" data-placement="top" title="{{ $project->pinComment->comment }}" style="cursor: pointer">
																			{{ $project->$header ??__('Not Provided') }}
																		</span>
																	</td>
																@else
															...
															<td>
																<div class="d-flex align-items-center justify-content-end action-btns-wrapper">
																	@if ($user_action_edit || $role == 's_admin')
																		<a href="{{ route('files.index',$project->id) }}" class="primary-btn primary-btn--blue w-auto d-inline-flex justify-content-center align-items-center rounded px-4">
																			{{ __('Update') }}
																		</a>
																	@endif
																</div>
															...
															@foreach ($filter_status as $item)
																...
																@elseif ($header == 'precariousness')
																	...
																	@else  
																		<td  id="{{ $header }}{{ $project->id }}">{{ $project->$header ??__('Not Provided') }}</td>
																	@endif 
																@elseif ($header == 'Nom' && $project->pinComment)
																	<td id="{{ $header }}{{ $project->id }}">
																		<span data-toggle="tooltip" data-placement="top" title="{{ $project->pinComment->comment }}" style="cursor: pointer">
																			{{ $project->$header ??__('Not Provided') }}
																		</span>
		...
		<div class="modal modal--aside fade" id="similarProjectModal" tabindex="-1" aria-labelledby="rightAsideModalLabel" aria-hidden="true">
				...
				<div class="modal-content border-0">
					...
					<div class="modal-body">  
						<div class="form-group">
							...
							<select class="select2_select_option form-control w-100" id="projectSimilarType" required>
						...
						<div class="text-center">
							<button type="button" id="projectSimilarCheck" class="primary-btn primary-btn--primary primary-btn--lg rounded-pill d-inline-flex align-items-center justify-content-center border-0">
								{{ __('Submit') }}
							</button>
						</div>
						<hr>
						...
						@if ($role == 's_admin')
							<div class="text-right">
								<button type="button" data-toggle="modal" data-target="#similarProjectBulkDelete" id="similarProjectBulkDeleteBtn" class="primary-btn btn-danger primary-btn--md rounded border-0 m-2 shadow-none d-none">Supprimer</button>	
							</div>
						@endif
						<div class="database-table-wrapper bg-white">
							<div class="table-responsive  simple-bar">
								<table class="table dynamic-table" id="projectSimilarTableBody">
									<thead>
										<tr> 
											<th></th>
                                            <th>ID</th>
                                            ...
                                            @if (checkAction(Auth::id(), 'project', 'similar-chantier-link') || $role == 's_admin')
												<th>LIEN</th>  
											@endif
										</tr>
									</thead>
			...
			<div class="modal-dialog modal-dialog-centered">
				<div class="modal-content">
					...
					<div class="modal-body text-center pt-0">
						...
						<span>{{ __('Êtes-vous sûr de créer un nouveau chantier') }} ?</span>
		...
		<div class="modal modal--aside fade" id="similarProjectBulkDelete" tabindex="-1" aria-labelledby="similarProjectBulkDeleteModal" aria-hidden="true">
			<div class="modal-dialog modal-dialog-centered">
				<div class="modal-content">
					<div class="modal-header border-0 pb-0">
						<button type="button" class="close d-inline-flex align-items-center justify-content-center rounded-circle m-0 p-0 ml-auto" data-dismiss="modal" aria-label="Close">
					...
					<div class="modal-body text-center pt-0">
						...
						<form action="{{ route('similar.project.bulk.delete') }}" method="POST">
							@csrf
							<input type="hidden" name="similar_project_bulk_id" id="similar_project_bulk_id" value="">
							<div class="d-flex justify-content-center">
								<button type="button" class="primary-btn primary-btn--orange primary-btn--md rounded border-0 my-3 mx-1" data-dismiss="modal" aria-label="Close">
									Annuler
								</button>
		...
		<!-- Middle Modal -->
		<div class="modal modal--aside fade" id="projectGestaionnaire" tabindex="-1" aria-labelledby="projectGestaionnaireLabel" aria-hidden="true">
			<div class="modal-dialog modal-dialog-centered">
				<div class="modal-content border-0">
					<div class="modal-header border-0 pb-0">
						<button type="button" class="close d-inline-flex align-items-center justify-content-center rounded-circle m-0 p-0 ml-auto" data-dismiss="modal" aria-label="Close">
					...
					<div class="modal-body pt-0">
						<form action="{{ route('project.gestionnaire.assign') }}" method="POST" class="form mx-auto needs-validation" novalidate>
							@csrf
							<h1 class="form__title position-relative text-center mb-4">Attribuer gestionnaire</h1>
							<div class="form-group d-flex flex-column align-items-center position-relative" id="bu">
								<input type="hidden" name="checkedProject" class="bulk_selected_project" value="">
		...
		<!-- Middle Modal -->
		<div class="modal modal--aside fade" id="projectTelecommercial" tabindex="-1" aria-labelledby="projectTelecommercialLabel" aria-hidden="true">
			<div class="modal-dialog modal-dialog-centered">
				<div class="modal-content border-0">
					<div class="modal-header border-0 pb-0">
						<button type="button" class="close d-inline-flex align-items-center justify-content-center rounded-circle m-0 p-0 ml-auto" data-dismiss="modal" aria-label="Close">
					...
					<div class="modal-body pt-0">
						<form action="{{ route('project.telecommercial.assign') }}" method="POST" class="form mx-auto needs-validation" novalidate>
							@csrf
							<h1 class="form__title position-relative text-center mb-4">Attribuer un télécommercial</h1>
							<div class="form-group d-flex flex-column align-items-center position-relative" id="bu" >
								<input type="hidden" name="checkedProject" class="bulk_selected_project" value="">
								<select class="select2_select_option form-control w-100" name="user_id" required>
		...
		<div class="modal modal--aside fade" id="projectBulkDeleteModal" tabindex="-1" aria-labelledby="middleModalLabel" aria-hidden="true">
			<div class="modal-dialog modal-dialog-centered">
				<div class="modal-content border-0">
					<div class="modal-header border-0 pb-0">
						<button type="button" class="close d-inline-flex align-items-center justify-content-center rounded-circle m-0 p-0 ml-auto" data-dismiss="modal" aria-label="Close">
					...
					<div class="modal-body text-center pt-0">
						<h1 class="form__title position-relative mb-4">{{ __('Confirmation') }}</h1>
						<span>{{ __('Are You Sure To Delete this') }} ?</span>
						<form  action="{{ route('projects.bulk.delete') }}" method="post">
							@csrf
							<input type="hidden" name="project_id" class="bulk_selected_project" value="">
							<div class="d-flex justify-content-center">
								<button type="button" class="primary-btn primary-btn--orange primary-btn--md rounded border-0 my-3 mx-1" data-dismiss="modal" aria-label="Close">
									Annuler
								</button>
		...
		<div class="modal modal--aside fade" id="projectImportModal" tabindex="-1" aria-hidden="true">
			...
			<div class="modal-content border-0">
				...
				<div class="modal-body pt-0">
					<form action="{{ route('projects.import') }}" class="form" id="projectFileImportForm" method="POST" enctype="multipart/form-data">
						@csrf
